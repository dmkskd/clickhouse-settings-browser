  ...
  function applyGroupTooltips() {
    ...
    if (els.scopeGroup) {
      Array.from(els.scopeGroup.querySelectorAll('.toggle')).forEach(btn => {
        ...
        if (v === 'session') btn.title = 'Session/query settings (system.settings)';
        else if (v === 'mergetree') btn.title = 'MergeTree storage settings (table-level)';
        else if (v === 'format') btn.title = 'Input/Output format settings';
      });
    }
    // Category
    if (els.categoryGroup) {
      Array.from(els.categoryGroup.querySelectorAll('.toggle')).forEach(btn => {
        btn.title = `Category: ${btn.textContent}`;
      });
    }
    // Tier with docs
    const tierDoc = 'https://clickhouse.com/docs/about-us/beta-and-experimental-features';
    if (els.tierGroup) {
      Array.from(els.tierGroup.querySelectorAll('.toggle')).forEach(btn => {
        const v = (btn.dataset.value || '').toLowerCase();
        if (v === 'production') btn.title = 'Production: safe to use with other production features';
        else if (v === 'beta') btn.title = `Beta: stable but interactions may be unknown — ${tierDoc}`;
        else if (v === 'experimental') btn.title = `Experimental: in active development — ${tierDoc}`;
      });
    }
  }
